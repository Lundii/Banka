...
describe('Staff can Activate or deactivate an account Number', function () {
  it('should return a status 200 if the account is successfully de-activated if activated initially', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/1004875498")).send(body2).set('Authorization', token).end(function (er, resp) {
  ...
  it('should return a status 400 if the account is already active', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/1004848398")).send(body2).set('Authorization', token).end(function (er, resp) {
  ...
  it('should return a status 200 if the account is successfully activated if de-activated initially', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/1004839098")).send(body2).set('Authorization', token).end(function (er, resp) {
        expect(resp).to.have.status(200);
        expect(resp).to.be.a('object');
        expect(resp.body).to.have.all.keys('status', 'data');
        expect(resp.body.data).to.include.all.keys('accountNumber', 'status', 'message');
  ...
  it('should return a status 400 if the account is already dormant', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/1004837498")).send(body2).set('Authorization', token).end(function (er, resp) {
        expect(resp).to.have.status(400);
        expect(resp).to.be.a('object');
        expect(resp.body).to.have.all.keys('status', 'error');
        expect(resp.body.error).to.be.a('String');
  ...
  it('should return a status 401 if the user is not a staff or admin', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/100495432")).send(body2).set('Authorization', token).end(function (er, resp) {
  ...
  it('should return a status 400 if account does not exit', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).patch("/api/v1/staff/".concat(id, "/account/107495432")).send(body2).set('Authorization', token).end(function (er, resp) {
        expect(resp).to.have.status(400);
        expect(resp).to.be.a('object');
        expect(resp.body).to.have.all.keys('status', 'error');
        expect(resp.body.error).to.be.a('String');
