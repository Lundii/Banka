...
describe('Admin can view all dormant/active bank accounts', function () {
  it('should return a status 200 if the request is successful', function (done) {
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).get("/api/v1/admin/".concat(id, "/accounts?status=active")).set('Authorization', token).end(function (er, resp) {
  ...
  it('should return a status 400 if query parameter \'status\' is not equal dormant', function (done) {
    var body = {
      email: 'onumonday@gmail.com',
      password: 'password'
    };
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).get("/api/v1/admin/".concat(id, "/accounts?status=allaccounts")).set('Authorization', token).end(function (er, resp) {
  ...
  it('should return a status 400 if query parameter is not status', function (done) {
    var body = {
      email: 'onumonday@gmail.com',
      password: 'password'
    };
    ...
    _chai["default"].request(_server["default"]).post('/api/v1/auth/signin').send(body).end(function (err, res) {
      ...
      _chai["default"].request(_server["default"]).get("/api/v1/admin/".concat(id, "/accounts?type=active")).set('Authorization', token).end(function (er, resp) {
        expect(resp).to.have.status(400);
        expect(resp).to.be.a('object');
        expect(resp.body).to.have.all.keys('status', 'error');
        expect(resp.body.error).to.be.a('String');
  ...
  it('should return a status 401 is user does not have a valid or expired token', function (done) {
    ...
    _chai["default"].request(_server["default"]).get("/api/v1/admin/".concat(id, "/accounts?status=active")).set('Authorization', 'ghjklldiOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmaXJzdE5hbWUiOiJtb25kYXkiLCJlbWFpbCI6Im1vbmRheXR1ZXNkYXlAZ21haWwuY29tIiwiaWF0IjoxNTU0OTM3Njc4LCJleHAiOjE1NTQ5NDQ4NzgsImlzcyI6Im1vbmRheS5sdW5kaWkifQ.XBP-AmW9ssM6T3GYeQIY-GUGMu7vjR2bbXey3Hc0dUU').end(function (err, res) {
