    ...
    this.store.userStore.read({ email: req.payload.email }, (err, result) => {
      ...
      if (result && result.length && result[0].emailconfirmed === false) {
        ...
        return res.status(403).json({
          status: 403,
          message:
            'Please click on the link sent to your mail to confirm your email before creating an account',
        });
      }
    ...
    this.store.bankAcctStore.read(
      ...
      (err, result) => {
        ...
        this.store.transactionStore.create(data, (err1, result1) => {
          ...
          this.store.bankAcctStore.update(
            { accountNumber: req.params.accountNumber },
                ...
                (err3, result3) => {
                  ...
                  if (!result3.length) {
                    return res.status(400).json({
                      status: 400,
                      error:
                        'Receiver account number does not exit in the database',
                    });
                  }
                  const data2 = {
                    createdOn: new Date(),
                    ...
                    newBalance:
                      result3[0].balance + parseFloat(req.body.transferAmount),
                  };
  ...
  uploadPassport(req, res) {
    this.store.userStore.update(
      ...
      { avatar: req.file.path }, (err, file) => {
        if (err) throw new Error('Error uploading passport');
        res.status(200).json({
          status: 200,
          file: {
            file: file[0].avatar,
          },
          message: 'Passport successfully uploaded',
        });
      },
    );
  }
