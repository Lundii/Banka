...
describe('Admin can view all staffs in the database', () => {
  it('should return a status 200 if the request is successful', (done) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        chai.request(server)
          .get(`/api/v1/admin/${id}/users?type=staff`)
          ...
          .end((er, resp) => {
            ...
            expect(resp.body.data[0].isadmin).to.equals(false);
            done();
          });
      });
  });
  it('should return a status 400 if query parameter \'type\' is not equal staff', (done) => {
    const body = {
      email: 'onumonday@email.com',
      password: 'password',
    };
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        const { token, id } = res.body.data;
        chai.request(server)
          .get(`/api/v1/admin/${id}/users?type=active`)
          .set('Authorization', token)
          .end((er, resp) => {
  ...
  it('should return a status 400 if query parameter is not type', (done) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        const { token, id } = res.body.data;
        chai.request(server)
          .get(`/api/v1/admin/${id}/users?status=active`)
          .set('Authorization', token)
          .end((er, resp) => {
            expect(resp).to.have.status(400);
            expect(resp).to.be.a('object');
