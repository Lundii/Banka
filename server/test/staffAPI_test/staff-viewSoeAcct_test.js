...
describe('Staff can view specific user (client) bank accounts', () => {
  it('should return a status 200 if the request is successful', (done) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        chai.request(server)
          .get(`/api/v1/staff/${id}/aishalawal23@email.com/accounts`)
  ...
  it('should return a status 401 is user does not have a valid or expired token', (done) => {
    ...
    chai.request(server)
      .get(`/api/v1/staff/${id}/amaka.padi@email.com/accounts`)
