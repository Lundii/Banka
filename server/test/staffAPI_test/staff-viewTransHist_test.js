...
describe('Staff can view account transaction history', () => {
  it('should return a status 200 if the request is successful', (done) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        chai.request(server)
          .get(`/api/v1/staff/${id}/accounts/1004848398/transactions`)
  ...
  it('should return a status 401 if the client is not a staff', (done) => {
    const body = {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        const { token, id } = res.body.data;
        chai.request(server)
          .get(`/api/v1/staff/${id}/accounts/1004837498/transactions`)
          .set('Authorization', token)
          .end((er, resp) => {
